...
public class RecipeConvertDtoToDto implements Converter<RecipeConvertDto, SaveRecipeDto> {
    ...
    public SaveRecipeDto convert(RecipeConvertDto source) {
        return new SaveRecipeDto(source.recipeEntity().getUuid(), source.recipeEntity().getDtCreate(),
                ZonedDateTime.of(source.recipeEntity().getDtUpdate(), ZoneId.systemDefault()).toInstant().toEpochMilli(),
                source.recipeEntity().getTitle(), source.ingredient(), source.recipeCPFCDto().weight(), source.recipeCPFCDto().calories(),
                source.recipeCPFCDto().proteins(), source.recipeCPFCDto().fats(), source.recipeCPFCDto().carbohydrates());
    }
}
